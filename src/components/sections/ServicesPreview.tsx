        ...
        <section className="relative bg-white border-b border-neutral-200">
            ...
            <div className={`grid grid-cols-1 md:grid-cols-3 opacity-0 ${isLoaded ? 'hero-animate-in hero-stagger-2' : ''}`}>
                {services.map((service, i) => (
                    ...
                    >
                        <div className="text-[0.65rem] text-[#ff3333] mb-4 font-light">{service.num}</div>
                        <h3 className="font-majesty text-xl text-black/90 mb-3">{service.title}</h3>
                        <p className="text-[0.82rem] leading-[1.7] text-neutral-500">{service.body}</p>
                        <Link
                            href="/services"
                            className="text-xs uppercase tracking-[0.1em] text-[#ff3333] hover:underline mt-5 inline-block font-light"
                        >
                            Explore →
                        </Link>
                    </div>
